    ...
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height; y++) {
        if (heightMap[x][y] < waterThreshold) {
          add(WaterModel(scheduler: _patchScheduler, cell: IntVector2(x, y)));
        } else {
          add(SoilModel(scheduler: _patchScheduler, cell: IntVector2(x, y)));
        }
      }
    }
/*
  ...
  AgentModel? getAgentAtCell(IntVector2 cell, String typeId) {
  ...
  void _grow() {
    ...
    void placePlants(int total, Function(IntVector2) placeFunction, {int radius = 1}) {
      final random = Random();
      for (int i = 0; i < total; i++) {
        ...
        for (int dx = -radius; dx <= radius && !placed; dx++) {
          for (int dy = -radius; dy <= radius && !placed; dy++) {
            int newX = ((x + dx + _size.x) % _size.x).toInt(); // Ensure wrapping around the grid
            int newY = ((y + dy + _size.y) % _size.y).toInt();
